function TableToolbar() {
  return (
    <div className="flex flex-wrap items-center gap-2 text-[14px] text-[#464E5F]">
      <span>Show</span>
      <div className="flex h-8 w-16 items-center justify-end rounded-md border-2 px-2">
        <svg xmlns="http://www.w3.org/2000/svg" width="10" height="6" viewBox="0 0 10 6" fill="none">
          <path d="M1 1L5 5L9 1" stroke="#464E5F" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </div>
      <span>Entries</span>
      <form className="flex items-center rounded-lg border-2" onSubmit={(e) => e.preventDefault()}>
        <input type="text" name="search" placeholder="Search" className="w-[150px] px-2 py-1 outline-none" />
        <svg
          xmlns="http://www.w3.org/2000/svg"
          className="w-8 cursor-pointer pr-1"
          viewBox="0 0 101 101"
          aria-hidden
        >
          <path d="M63.3 59.9c3.8-4.6 6.2-10.5 6.2-17 0-14.6-11.9-26.5-26.5-26.5S16.5 28.3 16.5 42.9 28.4 69.4 43 69.4c6.4 0 12.4-2.3 17-6.2l20.6 20.6c.5.5 1.1.7 1.7.7.6 0 1.2-.2 1.7-.7.9-.9.9-2.5 0-3.4L63.3 59.9zm-20.4 4.7c-12 0-21.7-9.7-21.7-21.7s9.7-21.7 21.7-21.7 21.7 9.7 21.7 21.7-9.7 21.7-21.7 21.7z" />
        </svg>
      </form>
    </div>
  )
}

export default TableToolbar
